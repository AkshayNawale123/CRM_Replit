...
- **Stage Timeline Tracking**: Automatic tracking of stage entry/exit times via `client_stage_history` table. Records when clients move between pipeline stages (not status changes), calculates duration in each stage, and provides analytics on sales velocity and bottlenecks. API endpoints: `/api/analytics/stages` for aggregate metrics, `/api/clients/:id/stage-history` for client-specific timeline.
  - **Duration Calculation Priority**: 1) Use `durationSeconds` if positive (trusted completed measurement), 2) Calculate from `enteredAt` to `exitedAt` for completed stages without `durationSeconds`, 3) Calculate from `enteredAt` to now for ongoing stages (showing "(ongoing)" suffix).
  - **Overdue Alerts**: `OverdueBadge` component displays warning/overdue status when current stage exceeds expected timeframes. Uses `pipelineStartDate` (or `createdAt` fallback) to accurately determine stage entry time.
  - **Stage Expected Durations**: Lead (7 days), Qualified (5 days), Proposal (10 days), Negotiation (14 days), Won/Lost (no limit).
